...
class PlgUniformPayment_Paypal extends JPlugin
{
	 ...
	 * @param string $paymentStatus		string of status
	 */
	public function buildNewPaymentStatusQueryFilter(&$query, $paymentStatus)
	{
	    $query->select('pm.*');
	    $query->join('INNER', '#__jsn_uniform_payment_paypal_logs AS pm ON pm.submission_id = sd.submission_id');
	    ...
	    {
	        $query->where('pm.log_status != "cancel"');
	    }
	 ...
	 * @param $submission_id
	 */
	public function updatePaymentLog($submissionID, $status)
	{
	    $db = JFactory::getDbo();
	    $query = $db->getQuery(true);

	    $fields = array(
	        $db->quoteName('log_status') . ' = ' . $db->quote($status),
	    );
	    $conditions = array(
	        $db->quoteName('submission_id') . ' = ' . $submissionID
	    );
