...
class JSNUniformHelper
{
    ...
    public static function generateStylePages($formStyle, $container = "", $containerActive = "", $title = "", $messageErrors = "", $messageBackgroundErrors = "", $field = "")
    {
        if (!empty($container))
        {
            ...
            {
                $styleCustom[] = "border:{$formStyle->border_color};";
            }
    ...
    public static function generateHTMLPages($formId, $formName, $formType = "", $topContent = "", $bottomContent = "", $showTitle = false, $showDes = false, $isModule = false, $preview = false, $itemPreview = "", $formPagesPreview = "")
    {
        ...
        if ($items)
        {
            ...
            if (!$formType)
            {
                ...
                $html .= "<form data-current-date='{$now}' data-token='{$ftoken}' data-ajaxupload='{$ajaxUpload}' id='form_{$formName}' action=\"" . 'JSN_UNIFORM_ROOT_URL' . '/index.php?option=com_uniform&amp;view=form&amp;task=form.save&amp;form_id=' . $items->form_id . "\" method=\"post\" class=\"form-validate {$classForm} \" enctype=\"multipart/form-data\" autocomplete=\"{$autoComplete}\">";
	        ...
	        $previewCustomClass = !empty($formSettings->form_btn_preview_custom_class) ? $formSettings->form_btn_preview_custom_class : "";
            ...
            {
                $htmlBtnReset = '<button type="reset" ' . $disabled . ' class="' . $btnResetStyle . ' reset ' . $resetCustomClass . '" style="display:inline-block">' . JText::_($btnReset) . '</button>';
            }
            else{
                $htmlBtnReset = '<button type="reset" ' . $disabled . ' class="' . $btnResetStyle . ' reset ' . $resetCustomClass . '" style="display:none">' . JText::_($btnReset) . '</button>';
            }

            $htmlBtnPreview = "";
            if (!empty($formSettings->form_state_btn_preview_text) && $formSettings->form_state_btn_preview_text == "Yes")
            {
                $htmlBtnPreview = '<button type="button" ' . $disabled . ' class="' . $btnPreviewStyle . ' preview ' . $previewCustomClass . '" style="display:inline-block">' . JText::_($btnPreview) . '</button>';
            }
            else
            {
                $htmlBtnPreview = '<button type="button" ' . $disabled . ' class="' . $btnPreviewStyle . ' preview ' . $previewCustomClass . '" style="display:none">' . JText::_($btnPreview) . '</button>';
            }
            ...
            if (!$formType)
            {
                ...
                $html .= "<input type=\"hidden\" name=\"ajax_upload_file\" value=\"{$ajaxUpload}\" />";
